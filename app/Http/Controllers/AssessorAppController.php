...
/*	private function GetFitnessLevel($Student_id, $TestTypeMasterID, $Score, $ScoreCritera) {
	...
	public function TestStatusHigherClass(Request $request)  {
        ...
        $school = School::find($schoolId);
		$higherClass = [4,5,6,7,8,9,10,11,12];
        ...
        if ($request->ajax()) {
            ...
            ->take($recordsTotal)->get();
	
	        $flattenedList = $studentlist->map(function ($item) use ($role_id) {

	        	if($role_id == 3){
	        		$view_link = route('trainer.reports.view', ['id' => $item->id]);
	        	}

				if($role_id == 4 || $role_id == 2){
					$view_link = route('reports.view', ['id' => $item->id]);
	        	}
	...
	public function TestStatusLowerClass(Request $request){
	    ...
	    if ($request->ajax()) {
			...
			$studentlist = $query->skip($start)->take($recordsTotal)->get();



	        $flattenedList = $studentlist->map(function ($item) use ($role_id) {
	            $view_link = $role_id == 3
	                ? route('trainer.reports.view', ['id' => $item->id])
	                : route('reports.view', ['id' => $item->id]);
