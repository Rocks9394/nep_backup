{{-- <footer>
    <div class="container">
        <div class="row">
            <div class="col">
               © Sequoia Fitness and Sports Technology Pvt. Ltd. All rights reserved.
            </div>
        </div>
    </div>
</footer> --}}
...
function getskillarea(i, val) 
    ...
    function getViewDartStudent(i, val) 
    {
        ...
        type: 'GET',
            success: function(response) 
            {
                ...
                {
                    var sno = key+1;
                    var pathreport = "{{route('students.report')}}?sid="+val.id;
    ...
    function getFillDARTskillarea(i, val) 
    {
        ...
        jQuery.ajax({
            url: "{{ route('getFillDARTSkillArea') }}",
            data: 
            {
                "school_id": school_id,
                "date_id"  : date_id,
