							...
							<form class="form-inline fltr-row " type="get" action="{{ route('admin.concepts.index') }}">				
							   ...
							   <?php if(!empty($_GET['name'])){ $name = $_GET['name'];}else{ $name='';  } ?> 
								...
								<div class="fltr-srch form-group">                	 
										<input type="search" name="name" value="<?=$name?>" id="name" class="form-control  fltr-txt-bx @error('name') is-invalid @enderror"  placeholder="Search Concept" width="180px">
					...
					<div class="col-md-12">
						<div class="row">
							...
							<button class="btn btn-success btn-sm dwl" name="download" value="download"
							 onclick="window.location.href='concept_export?class={{ request()->input('aclass')}}&subject={{ request()->input('subject')}}&chapter={{ request()->input('chapter')}}&search=search';" >
							<i class="fa fa-download"></i> Download</button>
							</div>
						

							<form action="{{ route('admin.updateconcept') }}" method="POST" enctype="multipart/form-data">
							@csrf
							...
							<tr>
								...
								<td>
									@if(Illuminate\Support\Str::contains( $cls->image,'storage/app/public/uploads'))
										<img src="{{ $cls->image }}" width="100" height="80">
